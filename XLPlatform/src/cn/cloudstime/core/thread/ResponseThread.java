...
public class ResponseThread {
	...
	public void run()
	{
		for(int i = 0; i < Global.RESPONSE_THREAD_COUNT; ++i) {  
                ...
                public void run() {  
                	while(true)
                	{
                		while(true)
                    	{
                    	 ...
                    	 {
                    		try {
    								Thread.sleep(Global.RESPONSE_THREAD_SLEEPTIME);
    								continue;
    							} catch (Exception e) {
    								e.printStackTrace();
    								//未完成
                    	 ...
                    	 else
                    	 {
                    		 ...
                    		 String status_desc=request.has("transaction_error_info")?request.getString("transaction_error_info"):"充值成功";
                    		 String trade_no=request.getString("transaction_code");
                    		 Integer trade_status=request.has("transaction_error_code")?request.getInt("transaction_error_code"):1;                   		
                    		 ...
                    		 //签名前
                    		 String str="business_type="+business_type+"&card_no="+card_no+extStr+"&notify_id="+notify_id+"&notify_time="+notify_time+
                                		 "&notify_type="+notify_type+"&order_no="+order_no+"&status_desc="+status_desc+
                                		 traStr+"&trade_status"+trade_status+"&key="+MD5Utils.MD5(key);
